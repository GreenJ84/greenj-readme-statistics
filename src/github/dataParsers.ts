...
import { LanguageData, RawStreakData, RawUserLanguages, RawUserStats, UserLanguages, UserStats, UserStreak } from "./types";

import { calculateGithubRank } from "./utils";

export const streakParse = (raw_data: RawStreakData, streak: UserStreak): UserStreak => {
  ...
  for (let week of raw_data.user.contributionsCollection.contributionCalendar
    .weeks) {
    if (!past) {
      break;
    }
...
export const statsParse = (data: RawUserStats): UserStats => {
  ...
  return { ...stats, grade: calculateGithubRank(stats) };
};

export const langsParse = (data: RawUserLanguages): UserLanguages => {
  let langs: Record<string, LanguageData> = {};
